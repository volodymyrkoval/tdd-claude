...
**Design-rubric is mandatory for Medium/Complex plans.** Before finalizing, invoke the `design-rubric` Skill via the Skill tool and answer the Section 7 self-critique questions explicitly in your thinking — the plan is not ready until each question has a concrete answer. Use Sections 2–6 to guide component boundaries, interface shape, and dependency direction. **Apply Q1 ("one reason to change") at both component AND method level.** For every method named in the plan (especially `render()`, `mount()`, `handle*()`, `process*()`-shaped methods), list the helpers it orchestrates. A method with zero helpers whose prescription touches more than one concern is a god-method waiting to happen — decompose it in the plan before the dev agent inherits the conflation. Component-level SRP is not sufficient if a single method inside a clean component fans out into 3+ unrelated responsibilities.
...
**`Explore` is a built-in `subagent_type` shipped with Claude Code — it is always available.** Do not check `.claude/agents/` for it, do not warn that it is missing, do not fall back to inline synthesis. If a Complex or `--deep` plan reaches this step, dispatch the four Agent calls below. Inline synthesis is not an acceptable substitute — the whole point of this step is *independent* parallel perspectives, which a single model writing the plan cannot produce.

Spawn four perspective explorations IN PARALLEL via the Agent tool with `subagent_type: "Explore"` — one per persona. The four personas (minimalist, extensibility, devils-advocate, user-advocate) are not agent files; each call passes the persona's framing in the `prompt` field. Example:
...
- **Todos:** Each = one testable behavior, ordered by dependency, with success/failure criteria. For each logical area, include at least one todo that explicitly covers edge cases (empty input, boundary values, error paths, invalid input) — don't leave them implicit for the dev agent to discover. Known edge cases become explicit todos; unknown ones surface during TDD triangulation.
  ...
  **Section structure rule — non-negotiable:** A tier group is the executor's unit of invocation; a todo is the group's unit of verification. Every section in the Todos block must open with a `#### Section briefing` subsection before the first tier-group label. The briefing serves all groups in the section — each group's executor reads the same briefing and lands only its own todos, without re-reading any other part of the plan. It contains exactly five elements (element 2 is required for Medium/Complex plans; Simple plans may omit it):
  ...
  2. **Methods produced** — for every class or module-level entry point this section creates or modifies, list each method (public and private) with a one-line responsibility. Format: `ClassName.method(args) — verb-phrase responsibility.` If a method orchestrates helpers, write `ClassName.orchestrator() → #helperA() → #helperB() → #helperC()` and list each helper on its own line below. **Trigger:** if you cannot write each method's responsibility in a single line without "and"/"then"/"also", the method is not yet decomposed — split it before continuing. This element exists to surface §3 ("Orchestrate, don't conflate") violations structurally; a section that produces a single multi-purpose method must enumerate the helpers here, not in todo prose.
  3. **Design context the executor needs upfront** — copy the one or two sentences from Key design decisions, Data flow, or Technical notes that bear directly on this section. Do not paraphrase — copy. If the section implements a named design decision, state its number and key constraint verbatim.
  4. **Cross-section couplings** — an explicit list of every todo in this section that depends on or is constrained by a todo in a different section. Format: `G5 depends on J1: Right Arrow is claimed by the options-panel layer; G5 must consume only ArrowUp/ArrowDown.` If there are no couplings, write `None.` — never omit this element.
  5. **Section-level Red criterion** — one paragraph stating what "this section is done" looks like in test terms. Must be specific enough that a test suite could verify it mechanically (file mounts, signal fires, handler dispatches, DOM state). Not a vague success blurb.

  Write the briefing after the design sections are complete — it draws from Components, Interfaces, Key design decisions, and Technical notes, so it cannot be written cold. A briefing that restates design docs rather than briefing an executor has failed; a cross-section coupling that says "see other sections" instead of naming the exact todo and constraint has failed; a Methods-produced element that names a single multi-responsibility method (e.g. `render() — renders the whole section`) without listing helpers has failed and must be decomposed before the plan ships.
...
- **Effort & dev tier per todo:** Tag each todo with effort (S/M/L) and recommended dev agent:
  ...
  **Tier assignment process — do this for every todo:**
  ...
  3. **Orchestrate, don't conflate.** Decomposition into named helpers is mandatory whenever the prescription crosses ANY of these structural thresholds — count them mechanically, do not eyeball:

     - **2+ distinct UI primitives** mounted by one method (heading, dropdown, button, link, panel, row, modal, list, input).
     - **2+ distinct persistence writes** in one method (settings save, session-map put, vault write, dispatcher call, cache update).
     - **2+ distinct error-handling branches** (any `try/catch`, fallback path, or Notice-on-failure inside an otherwise-success-path method).
     - **2+ async operations** chained inside one handler.
     - **Prose backstop:** if you'd describe the prescription with "and"/"then"/"also" between verbs (`do A, then B, then C, then D`) — decompose, regardless of the counts above.

     When any threshold is crossed, rewrite the prescription as an orchestrator that calls named helpers (e.g. `processOrder(input)` orchestrates `validate(input)` → `price(...)` → `persist(...)` → `notify(...)`) and **list every helper by name in the prescription itself** — not buried inside Section briefing prose, not implied by a Section heading. The dev agent then implements decomposed code from the start, not after a refactor pass. Helpers and orchestrator both stay `junior-dev` — decomposition is mechanical, not design-shaping. This is the same `one thing at one level of abstraction` rule that `design-rubric` §3 applies at code-review time, pulled forward into planning.

     **Worked counter-example (the failure this rule prevents).** A prescription that reads `create CustomRefineSection with a single render() method. Dropdown, button, conditional Open button per Section briefing.` crosses two thresholds: 3 UI primitives (dropdown + create button + open button) and 1 error branch (the create handler wraps `seeder.seed()` in try/catch). The right prescription names the helpers: `render()` orchestrates `#renderHeading(el)` → `#renderActiveDropdown(el, deps)` → `#renderCreateButton(el, deps)` → `#renderOpenButton(el, deps)`; the create button's `onClick` delegates to `#handleCreateFromDefault(deps)` which carries the try/catch and the seed-then-refresh sequence. Five names, listed in the todo line. Decomposition closed at plan time.
